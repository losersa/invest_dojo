...
本目录采用 **产品 / 架构 / API / 决策记录 / 运维** 五层文档结构，分层独立维护。

> 🚨 **新接手请先看**：[ops/2026-05-02-本地栈部署完成.md](./ops/2026-05-02-本地栈部署完成.md) —— 数据库现在跑在 Windows 电脑上（Supabase Lite 精简栈），不是 Supabase Cloud。
...
├── ops/                              ← 运维与重大事件记录
│   ├── 运维手册.md                    ✅ 日常启停/排障/备份 速查
│   ├── 2026-05-02-本地栈部署完成.md    ✅ 当前数据库现状
│   └── 2026-05-01-停写说明.md          ⚠️ 已过时（保留作历史）
└── _archive/                         ← 归档
...
| 文件 | 状态 | 说明 |
|------|------|------|
| [运维手册.md](./ops/运维手册.md) | ✅ Active | 启停/排障/备份/升级路径 速查 |
| [2026-05-02-本地栈部署完成.md](./ops/2026-05-02-本地栈部署完成.md) | ✅ Done | 当前数据库部署在 Windows（Supabase Lite 精简栈） |
| [2026-05-01-停写说明.md](./ops/2026-05-01-停写说明.md) | ⚠️ Outdated | Supabase Cloud 停写期间的临时方案（已由上文取代） |

> **新接手的 agent 先看 `2026-05-02-本地栈部署完成.md`**，了解当前数据库不在云上，而在 Windows 电脑上跑着。
