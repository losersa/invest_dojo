...
function BacktestPanel() {
  ...
  // 支持从训练结果页「用此模型回测」带 model_id 直接调用
  const [modelId, setModelId] = useState("mock_momentum_v1");
  useEffect(() => {
    const p = new URLSearchParams(window.location.search).get("model_id");
    if (p) setModelId(p);
  }, []);
  ...
  const run = async () => {
    ...
    try {
      const r = await sdk.backtests.runFast({
        mode: "fast",
        strategy: { type: "model", model_id: modelId },
        start: "2023-01-01",
        end: "2023-06-30",
        universe: "hs300",
      });
  ...
  return (
    <div>
      <div style={{ display: "flex", gap: 8, alignItems: "center", marginBottom: 12, flexWrap: "wrap" }}>
        <label style={{ fontSize: 12, color: "#888" }}>
          ...
          <input
            ...
            placeholder="从训练结果页带 model_id 直接调"
          />
        </label>
      </div>
      <button
      ...
      >
        {loading ? "回测中..." : "跑一次回测（2023 上半年 / 沪深 300）"}
      </button>
