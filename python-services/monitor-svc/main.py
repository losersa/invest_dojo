...
from alerts import collect_alerts_overview
...
@router.get("/alerts", summary="分模块数据报表 + 告警聚合")
async def alerts():
    """按模块（infra/services/data/feature/train/backtest）聚合实时数据报表与告警。

    供 admin 告警页使用：每个模块返回 status/report/alerts，
    overall 取所有模块中最差的状态。
    """
    data = await collect_alerts_overview()
    return {
        "data": data,
        "meta": {
            "timestamp": datetime.now(UTC).isoformat(),
