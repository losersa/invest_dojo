...
async def list_factors(
    ...
    output_type: str | None = Query(None, pattern="^(boolean|scalar|rank)$", description="按输出类型过滤"),
    sort: str = Query("-updated_at", description="排序字段，- 开头表降序"),
    include_stats: bool = Query(True, description="是否包含 stats（关闭可加速）"),
    value_start: str | None = Query(
        None, description="has_values 判定的起始日期（YYYY-MM-DD），需与 value_end 同传"
    ),
    value_end: str | None = Query(
        None, description="has_values 判定的截止日期（YYYY-MM-DD），需与 value_start 同传"
    ),
    ...
    - `has_values` 标注：默认表示"全时段是否有预计算值"；同传 value_start/value_end
      则判定该区间内是否有值（训练页按训练区间过滤可用因子用）
    """
    ...
    # output_type 过滤（精确匹配枚举值）
    if output_type:
        filters["output_type"] = f"eq.{output_type}"
    ...
    # 标注每个因子是否有预计算的因子值（feature_values 有行 → 训练可用）。
    # 用主键 (factor_id, symbol, date) 索引做逐因子 EXISTS（select limit 1），
    # 每因子 ~1ms；切勿 select_all 拉全量行去重——该写法曾把本接口拖到 15s+。
    # 同传 value_start/value_end 时按日期区间判定（主键第三列范围，仍走索引）。
    if data:
        ...
        try:
            for row in data:
                ...
                hit = client.select(
                    "feature_values",
                    columns="factor_id",
                    filters={"factor_id": f"eq.{fid}", **value_filter},
                    limit=1,
                )
                if hit:
                    valued_ids.add(fid)
        except Exception:  # noqa: BLE001 - 标注失败不应阻断列表
            valued_ids = set()
        for row in data:
            row["has_values"] = row["id"] in valued_ids
