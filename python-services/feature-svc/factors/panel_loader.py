...
def _compute_derived(panel: dict[str, pd.DataFrame]) -> None:
    ...
    # klines 的 numeric 列从 Postgres 读出来是 decimal.Decimal，
    # 而基本面字段是 float；二者直接做算术会抛
    # "unsupported operand type(s) for /: 'decimal.Decimal' and 'float'"。
    # 衍生字段在交给 Engine（其 __init__ 才统一转 float64）之前就计算了，
    # 所以这里先把 close 转成 float64，否则 pb / pe_ttm 等衍生会因类型不兼容 500。
