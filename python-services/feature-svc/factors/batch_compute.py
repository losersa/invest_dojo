...
def compute_and_save(
    ...
    *,
    ...
    # 诊断性告警：只要算出 0 条就告警（不再依赖特定 error 字符串）。
    # 以前这里仅在 errors 含 "no kline data for batch" 时告警；panel 非空但
    # date_mask 全裁掉（K 线陈旧）的情形会"静默 0 记录"，运维难以察觉根因。
    if total_written == 0 and parsed and symbols:
        no_kline = any(
            e.get("error", "").endswith("no kline data for batch") for e in total_errors
        )
        logger.warning(
            "batch_compute.zero_records_written",
            start=start,
            end=end,
            factors=len(parsed),
            symbols=len(symbols),
            ...
            no_kline_in_range=no_kline,
            hint=(
                "本次计算 records_written=0。常见原因："
                "1) 目标区间内无 K 线（K 线未刷新，数据停留在 end 之前），date_mask 全裁掉；"
                "2) 全部因子在本批次报错（见 errors）；"
                "3) 因子结果全为 NaN。"
                "请确认 K 线数据已覆盖 start~end（scripts/update_5m_klines.py）。"
            ),
        )
