...
async def trigger_task(
    ...
    task_map = {
        "update_5m_klines": {
            "script": "update_5m_klines.py",
            "label": "增量更新 K 线（5m 拉取 + 聚合日K）",
            "tables": ["klines_all"],
        },
        "backfill_5m_klines": {
            "script": "update_5m_klines.py",
            "args": ["--from", "2026-05-01", "--to", datetime.utcnow().strftime("%Y-%m-%d")],
            "label": "回补 5m K线空缺段（2026-05 起）",
            "tables": ["klines_all"],
        },
        "update_klines": {
            "script": "update_daily_klines.py",
            "label": "增量更新日 K（旧·独立日K拉取，已由 5m 管线替代）",
            "tables": ["klines_all"],
        },
        "update_snapshots": {
